...
export function PostMetaSidebar({ meta, onChange, onSlugBlur, slugError }: PostMetaSidebarProps) {
  ...
  return (
    <>
      <div className="w-72 shrink-0 border-2 border-ink rounded-xl overflow-hidden bg-white sticky top-0 shadow-hard-xs">
          ...
          <Field label="Cover Image">
            {meta.cover_image_url ? (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="font-mono text-xs text-ink/50 hover:text-ink transition"
                  >
                    Change
                  </button>
                  <button
                    ...
                    className="font-mono text-xs text-ink/40 hover:text-brand-red transition"
                  >
                    Remove
                  </button>
                </div>
              ...
              <button
                type="button"
                onClick={() => setPickerOpen(true)}
                className="w-full aspect-video flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-ink/20 bg-ink/3 text-ink/40 hover:border-ink/40 hover:text-ink transition"
              >
                <ImageIcon size={20} />
      ...
      <MediaPickerModal
        ...
        onSelect={(url) => set('cover_image_url', url)}
      />
    </>
  )
}
