...
export function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) navigate('/link-error?reason=invalid', { replace: true })
  }, [token, navigate])
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      // Backend issued a session cookie — refresh context before navigating
      // so ProtectedRoute sees the authenticated user instead of null.
      await refresh()
      ...
      if (isAxiosError(err)) {
        ...
        if (status === 401) {
          ...
          setFormError('Password reset is unavailable right now. Use the magic link sign-in from the login page instead.')
        } else {
